...
float accX = 0.0F, accY = 0.0,accZ = 0.0F;  // Define variables for storing inertial sensor data
//float accY = 0.0F;  
//float accZ = 0.0F;
bool acqprima,stato=false, completo_calib = false;
float accelerazione, alpha = 0.69,filtered_acc = 0, total = 0,avg, width, step_length,  accTot=0, mean ;
int numeroPassi=0, count, giri, alt, largh;
...
void loop()
 {
    ...
    // Calibration for average acceleration.
     ...
     if(filtered_acc>avg+width && completo_calib)
     {
       stato=true;
       acqprima=stato;
     }
